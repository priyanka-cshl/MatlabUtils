function [myFR] = MakeSniffTriggeredPSTH(AllSpikes, SniffDurations, WindowStart, varargin)
...
SniffDurations = ceil(1000*SniffDurations) - WindowStart;
Sniffcounts = zeros(1,max(SniffDurations));
for i = 1:numel(SniffDurations)
    Sniffcounts(1,1:SniffDurations(i)) = 1 + Sniffcounts(1,1:SniffDurations(i)); 
end

if numel(myPSTH) <= numel(Sniffcounts)
    myPSTH(:,(numel(myPSTH)+1):numel(Sniffcounts)) = 0;
else
    myPSTH(:,(numel(Sniffcounts)+1):end) = [];
end

% Make PSTH (raw)
myPSTH = myPSTH./Sniffcounts;
