function [htop,hbot] = anomaly(x,y,varargin) 
% anomaly plots line data with different colors of shading filling the area
...
% 
% A simple anomaly plot: 
% 
%    anomaly(x,y)
% 
% Remove line color and specify RGB values of top and bottom shading: 
% 
%    anomaly(x,y,'edgecolor','none',...
%       'topcolor',[0.4 0.7608 0.6471],...
%       'bottomcolor',[0.9882    0.5529    0.3843])
% 
% Sometimes anomalies are not relative to zero, but are relative to their mean: 
% 
...
% 
% See also: area, patch, and fill. 

%% Error checks: 

narginchk(2,inf) 
assert(numel(x)==numel(y),'Input error: The dimensions of x and y must agree.') 
assert(isvector(x)==1,'Input error: x and y must be vectors of the same dimension.') 
assert(issorted(x)==1,'Input error: x must be monatonically increasing.') 
...
if nargin>2
   ...
   % Check for base (reference or relative) value declaration: 
   tmp = strncmpi(varargin,'base',3); 
   if any(tmp) 
      base = varargin{find(tmp)+1}; 
      assert(isscalar(base)==1,'Input error: Base value must be a scalar.') 
      tmp(find(tmp)+1)=1; 
      varargin = varargin(~tmp); 
   end
   
   % Top face color: 
   tmp = strcmpi(varargin,'topcolor'); 
   if any(tmp) 
      topcolor = varargin{find(tmp)+1}; 
      tmp(find(tmp)+1)=1; 
      varargin = varargin(~tmp); 
   end
   
   % Bottom face color: 
   tmp = strncmpi(varargin,'bottomcolor',3); 
   if any(tmp) 
      bottomcolor = varargin{find(tmp)+1}; 
      tmp(find(tmp)+1)=1; 
      varargin = varargin(~tmp); 
   end   
end
...
% Plot the top half: 
htop = area(x,ytop,base,'facecolor',topcolor,varargin{:}); 

% Plot the bottom half: 
hbot = area(x,ybot,base,'facecolor',bottomcolor,varargin{:}); 
